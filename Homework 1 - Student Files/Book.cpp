...
///////////////////////// TO-DO (2) //////////////////////////////
  : _isbn{std::move(isbn)},             // Copy-move idiom
    _title{std::move(title)},           // Use the constructor initialization list
    _author{std::move(author)},
    _price{price}
    {}                                  // Avoid setting values in constructor's body
/////////////////////// END-TO-DO (2) ////////////////////////////
...
{
  ///////////////////////// TO-DO (15) //////////////////////////////
  _isbn = std::move(newIsbn);     // Same as setter
  return *this;
...
{
  ///////////////////////// TO-DO (16) //////////////////////////////
  _title = std::move(newTitle);   // Same as setter
  return *this;
...
{
  ///////////////////////// TO-DO (17) //////////////////////////////
  _author = std::move(newAuthor);   // Same as setter
  return *this;
...
std::istream & operator>>( std::istream & stream, Book & book )
{
  ...
  if(stream >> std::ws >> std::quoted(workingItem._isbn) && stream >> std::ws >> delimiter && delimiter == ',' &&     // intentionally using implicit conversions from stream to bool and
     stream >> std::ws >> std::quoted(workingItem._title) && stream >> std::ws >> delimiter && delimiter == ',' &&    // short-circuit boolean logic to stop as soon as an error is detected
...
std::ostream & operator<<( std::ostream & stream, const Book & book )
{
  ...
  stream << std::quoted(book.isbn()) << delimiter
         ...
         << book.price();
                                                          // Note: Writing the newline character is intentionally omitted
  return stream;
